...
- 一个房间同时只能有一个活动任务。
- 活动任务只在 `start` 事件获中转 `accepted` 后建立。并发 `start` 中只会接纳一个；被拒绝的一侧关闭自己的候选记录并接收获准任务。
- 非 `start` 类型的事件必须属于已知且未关闭的任务。
- 只有发起方可以发 `complete`。
- 已取消的任务只接受 `cancel`。
- 线路事件的 `sender` 必须是认证成员自己。

本机启动任务时先以 `pending` 记录原始 `start` 事件。只有收到并持久化中转 `accepted` 回执后，才可启动 Codex。明确拒绝会把候选任务标记为已退出；超时或断连则记为 `unknown`，恢复时重连并对账同一个事件。没有可核验回执的任务不能启动或恢复本机 Agent。收到中转投递的同伴 `start` 事件也证明该任务已获接纳。
