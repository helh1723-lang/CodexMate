...
const server=spawn(process.execPath,['dist/chat/cli.js','ui','--home',home,'--port','4318'],{stdio:'pipe',windowsHide:true});
server.stdout.on('data',b=>process.stdout.write(b));server.stderr.on('data',b=>process.stderr.write(b));
let ready=false;
try{
  for(let i=0;i<100;i++){if(server.exitCode!==null)throw new Error('UI server exited');try{const r=await fetch('http://127.0.0.1:4318/api/session');if(r.ok){ready=true;break;}}catch{}await new Promise(r=>setTimeout(r,100));}
