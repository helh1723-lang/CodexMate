...
export class RelayClient extends EventEmitter {
  ...
  connect(){
    ...
    ws.on('message',raw=>{if(this.stopped||this.ws!==ws)return;try{const m=JSON.parse(String(raw));
      ...
      if(m.type==='accepted'){this.db.transaction(()=>{this.db.put('relay-accepted',m.id,true);this.db.remove('outbox',m.id);});this.emit('delivered',m.id);}
  ...
  async accepted(id:string){if(this.db.get('relay-accepted',id))return;const failure=this.db.get<any>('rejected',id);if(failure)throw new Error(failure.error);if(!this.db.get('outbox',id))throw new Error('找不到待对账的中转事件，无法确认任务已接纳');
